...
void Heap::collect() 
{
    int markDescendants = empty, markedCells = empty;
    
    for (int p = 0; p < rootCnt; p++) 
    {
        if (roots[p] != empty) 
        {
    ...
    for (int p = markDescendants; p != empty; p = markDescendants) 
    {
    ...
    for (int p = markedCells; p != empty; p = next(p))
    {
        marked(p) = false;
    }
 ...
 if (rootCnt > 0)
    {
        ...
        {
            roots[p] = empty; // set variable to null;
        }
    }
}

void Heap::printList(int list, const char* name) 
{
    cout << name << ": ";
...
ostream& operator<< (ostream& out, Heap& h) 
{
    ...
    for (int i = 0; i < h.rootCnt; i++)
    {
        cout << h.roots[i] << " ";
    }
    cout << endl;
    for (int i = 0; i < h.maxHeap; i++)
    {
